...
def check_double_digits_exactly(password):
    password = str(password)
    pointer = 0
    while pointer < 5:
        if password.count(password[pointer]) == 2 and password[pointer] == password[pointer+1]:
            return True
        pointer += 1
    return False
...
def check_password(password):
    if check_length(password) and check_double_digits_exactly(password) and check_increasing(password):
        return True
    else:
        return False
